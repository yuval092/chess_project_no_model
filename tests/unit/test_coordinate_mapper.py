...
def test_square_to_world_corners_and_round_trip() -> None:
    ...
    assert np.allclose(mapper.square_to_world(chess.A1), [-0.245, -0.245, 0.77])
    assert np.allclose(mapper.square_to_world(chess.H8), [0.245, 0.245, 0.77])
    for square in chess.SQUARES:
        assert mapper.world_to_square(mapper.square_to_world(square)) == square
    assert mapper.world_to_square(np.array([2.0, 2.0, 0.77])) is None
